...
class TaskmanagerController extends Controller {
	...
	*/
	...
	public function task_manager()
	{
		...
		return view('user/task_manager/task_manager', array('title' => 'Easypayroll - Task Manager', 'userlist' => $user, 'taskslist' => $tasks,'user_tasks' => $user_tasks,'open_task_count' => $open_task_count,'authored_task_count' => $authored_task_count));
	}
	public function taskmanager_search()
	{
		...
		$tasks = DB::table('time_task')->where('task_type', 0)->orderBy('task_name', 'asc')->get();
		$user = DB::table('user')->where('user_status', 0)->where('disabled',0)->orderBy('firstname','asc')->get();
		return view('user/task_manager/task_search', array('title' => 'Easypayroll - Task Manager', 'userlist' => $user, 'taskslist' => $tasks,'user_tasks' => $user_tasks,'open_task_count' => $open_task_count,'authored_task_count' => $authored_task_count));
	}
	public function task_administration()
	{
		...
		$tasks = DB::table('taskmanager')
				->join('user', 'taskmanager.author', '=', 'user.user_id')
				->select('taskmanager.id','taskmanager.taskid','taskmanager.client_id','taskmanager.task_type','taskmanager.author','taskmanager.allocated_to','taskmanager.subject','taskmanager.retain_specifics','taskmanager.retain_files','taskmanager.recurring_task','taskmanager.recurring_days','taskmanager.due_date','taskmanager.creation_date','taskmanager.status','user.firstname','user.lastname')->orderBy('id','desc')->limit(500)->get();
		$user = DB::table('user')->where('user_status', 0)->where('disabled',0)->orderBy('firstname','asc')->get();
		return view('user/task_manager/task_administration', array('title' => 'Easypayroll - Task Manager', 'taskslist' => $tasks,'userlist' => $user,'user_tasks' => $user_tasks,'open_task_count' => $open_task_count,'authored_task_count' => $authored_task_count));
	}
		...
		if(count($user_tasks))
	    {
	        foreach($user_tasks as $task)
	        {
	          ...
	          if($task->client_id == "")
	          {
	            ...
	            if(count($task_details))
	            {
	            	$title = '<spam class="task_name_'.$task->id.'">'.$task_details->task_name.'</spam> <a href="javascript:" class="edit_task_name fa fa-edit" data-element="'.$task->id.'" data-value="'.$task_details->task_name.'" style="padding: 5px;font-size: 20px;font-weight: 800;"></a>';
	            }
	            else{
	            	$title = '<spam class="task_name_'.$task->id.'"></spam> <a href="javascript:" class="edit_task_name fa fa-edit" data-element="'.$task->id.'" data-value="" style="padding: 5px;font-size: 20px;font-weight: 800;"></a>';
	            }
	          }
	          else{
	            $title_lable = 'Client:';
	...
	public function search_taskmanager_task()
	{
		...
		if($open_task != "0"){ 
			if($query == "") { $query.= "`status` = '0'"; } else { $query.= " AND `status` = '0'"; }
		}
		else{
			if($query == "") { $query.= "`status` = '1'"; } else { $query.= " AND `status` = '1'"; }
		}
		...
		if(count($user_tasks))
	    {
	        foreach($user_tasks as $task)
	        {
	          ...
	          if($task->client_id == "")
	          {
	            ...
	            if(count($task_details))
	            {
	            	$title = '<spam class="task_name_'.$task->id.'">'.$task_details->task_name.'</spam> <a href="javascript:" class="edit_task_name fa fa-edit '.$disabled.'" data-element="'.$task->id.'" data-value="'.$task_details->task_name.'" style="padding: 5px;font-size: 20px;font-weight: 800;"></a>';
	            }
	            else{
	            	$title = '<spam class="task_name_'.$task->id.'"></spam> <a href="javascript:" class="edit_task_name fa fa-edit '.$disabled.'" data-element="'.$task->id.'" data-value="" style="padding: 5px;font-size: 20px;font-weight: 800;"></a>';
	            }
	          }
	          else{
	            $title_lable = 'Client:';
	...
	public function show_more_tasks()
	{
		...
		$tasks = DB::table('taskmanager')
				->join('user', 'taskmanager.author', '=', 'user.user_id')
				->select('taskmanager.id','taskmanager.taskid','taskmanager.author','taskmanager.allocated_to','taskmanager.subject','taskmanager.retain_specifics','taskmanager.retain_files','taskmanager.recurring_task','taskmanager.recurring_days','taskmanager.due_date','taskmanager.creation_date','taskmanager.status','user.firstname','user.lastname')->orderBy('id','desc')->offset($offset)->limit(500)->get();
		$outputtask = '';
          if(count($tasks))
          {
            foreach($tasks as $task)
            {
              ...
              $outputtask.='<tr id="tr_task_'.$task->id.'" class="tr_task '.$tr_status.'">
                ...
                <td class="author_td" style="'.$color.'">'.$task->lastname.' '.$task->firstname.'</td>
